      ...
      <div class="block full">
        ...
        <?php
          if(isset($_POST['add'])){
            ...
            $busca_it_add = $db->prepare("SELECT Title FROM meh_news WHERE id='$id'");
             $busca_it_add->execute();
            $conta_it_add = count($busca_it_add->fetchAll( PDO::FETCH_BOTH ) );
            
            if(empty($nome)){
              echo "<b>Fill out all fields! New title or content or image Are Empty</b>";
            }else if($conta_it_add > 0){
              echo "<b>ID in use!</b> ";
            }else{
              $insert_item_db = $db->prepare("INSERT INTO meh_news (`id`, `Title`, `Content`, `Image`, `Date`) VALUES ('$id', '$nome', '$content', '$Image', NOW( ))");
...
setTimeout(function () { location.href = 'news.php';
}, 5000);
</script>";              }else{
...
<tr>
          ...
          <tr>
            <td><font>Content:</font><textarea placeholder="Type the content of the new." class="elastic form-control required" name="content" cols="40" rows="5"></textarea></td>

            <td class="pt-3 pl-3"><font>Image:</font><input type="text" class="datepicker-trigger form-control hasDatepicker" name="Image"></td>
          </tr>
          <tr id="upload">
            <td class="pt-3"><div class="block bg-dark"><a href="#" onclick="window.open('upload-images.php', 'Pagina', 'STATUS=NO, TOOLBAR=NO, LOCATION=NO, DIRECTORIES=NO, RESISABLE=NO, SCROLLBARS=YES, TOP=10, LEFT=10, WIDTH=770, HEIGHT=400');" class="text-info">Send .png</a></div>  </td>
          </tr>
      ...
      <div class="block full">
        ...
        <?php
          if(isset($_POST['edd'])){
            ...
            $ed_content = addslashes($_POST['ed_content']);
            $ed_Image = addslashes($_POST['ed_Image']);
            ...
            if(empty($ed_name)){
              ...
              $insert_db = $db->prepare("UPDATE meh_news SET Title='$ed_name', Content='$ed_content', Image='$ed_Image' WHERE id='$ed_id'");
              $insert_db->execute();
              if($insert_db){
...
setTimeout(function () { location.href = 'news.php';
        ...
        <?php
          ...
          $busca_edit = $db->prepare("SELECT * FROM meh_news WHERE id=$edit");
          $busca_edit->execute();
          ...
          if($conta_edit > 0){
            ...
            $edit_name = $fetch_edit['Title'];
            $edit_content = $fetch_edit['Content'];
            $edit_Image = $fetch_edit['Image'];
          }
        ?>
          <form action="" method="POST">
            <table>
              <tr>
                ...
                <td class="pt-3 pl-3"><font>Title:</font><input type="text" class="datepicker-trigger form-control hasDatepicker" name="ed_name" value="<?php echo $edit_name; ?>"></td>
              </tr>
              
              <tr>
               <td><font>Content:</font><textarea class="elastic form-control required" name="ed_content" cols="40" rows="5"><?php echo $edit_content; ?></textarea></td>
               <td class="pt-3 pl-3"><font>Image:</font><input type="text" class="datepicker-trigger form-control hasDatepicker" name="ed_Image" value="<?php echo $edit_Image; ?>"></td>
              </tr>
              <tr id="upload">
            <td class="pt-3"><div class="block bg-dark"><a href="#" onclick="window.open('upload-images.php', 'Pagina', 'STATUS=NO, TOOLBAR=NO, LOCATION=NO, DIRECTORIES=NO, RESISABLE=NO, SCROLLBARS=YES, TOP=10, LEFT=10, WIDTH=770, HEIGHT=400');" class="text-info">Send .png</a></div>  </td>
          </tr>
