        ...
        <h6 class="heading-hr"><i class="icon-file"></i>Edit item</h6>
          ...
          if(isset($_POST['edd'])){
            ...
            if(empty($ed_name) || ($ed_preco < 0) || ($ed_level < 0) || ($ed_estoque < 0) || ($ed_stk < 0)){
              ...
              $insert_db = $db->prepare("UPDATE meh_items SET `id`='$ed_id', `Name`='$ed_name', `Coins`='$ed_coins', `Upg`='$ed_Upg', `Temp`='$ed_temp', `Cost`='$ed_preco', `Lvl`='$ed_level', `QtyRemain`='$ed_estoque', `ReqItems`='$ed_reqitems', `File`='$ed_file', `Link`='$ed_link', `Desc`='$ed_desc', `Type`='$ed_type', `ES`='$es', `Icon`='$icon', `ItemRarity`='$ed_ItemRarity', `isTradeable`='$ed_tradeable', `Founder`='$ed_founder', `ReqQuests`='$ed_ReqQuest', `Staff`='$ed_staff', `Stk`='$ed_stk', `Date`=CURRENT_TIMESTAMP, `Author`='$useron', Race='$ed_race', isHuman='$human', isUndead='$undead', isElf='$elf', isBeast='$beast' WHERE `id`='$ed_id'");
              $insert_db->execute();
            if($insert_db) {
...
setTimeout(function () { location.href = 'items.php';
        ...
        ?>
          ...
          <tr>
            ...
            <td class="pt-3 pl-3">
              <font>Item Race:</font>
              <select name="ed_race" class="form-control mb-3 mb-3 bg-dark">
