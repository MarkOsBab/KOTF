        ...
        <?php
          if(isset($_POST['add'])){
            ...
            if(empty($nome) || empty($Items)){
              ...
              $insert_item_db = $db->prepare("INSERT INTO meh_items_shops (`id`, `Name`, `Items`, `Field`, `Staff`, `Founder`, `Upgrade`, `House`, `Limited`,`Human`,`Undead`,`Elf`,`Beast`) VALUES ('$id', '$nome', '$Items', '', '$Staff', '$Founder', '$Upgrade', '0', '$Limited', '$human', '$undead', '$elf', '$beast')");
...
<tr>
          ...
          <tr>
            ...
            <td class="pt-3 pl-3">
              <font>Item Race:</font>
              <select name="Race" class="form-control mb-3 mb-3 bg-dark">

                <?php if(isset($_POST['Race']) && $_POST['Race'] >= 0){ ?>
                  <option selected value="None">No Race</option>
                  <option value="Human">Human Race</option>
                  <option value="Undead">Undead Race</option>
                  <option value="Elf">Elf Race</option>
                  <option value="Beast">Beast Race</option>
                <?php }else{ ?>
                  <option selected value="None">No Race</option>
                  <option value="Human">Human Race</option>
                  <option value="Undead">Undead Race</option>
      ...
      <div class="block full">
        ...
        <?php
          if(isset($_POST['edd'])){
            ...
            $ed_race = addslashes($_POST['ed_race']);
            ...
            $beast = 0;
             ...
             if ($ed_race == 'None') {
                ...
                $elf = -1;
                $beast = -1;
              } elseif ($ed_race == 'Human') {
                $human  = 1;
                $undead = -1;
                $elf = -1;
                $beast = -1;
              } elseif ($ed_race == 'Undead') {
                $human = -1;
                $undead = 1;
                $elf = -1;
                $beast = -1;
            ...
            if(empty($ed_name)){
              ...
              $insert_db = $db->prepare("UPDATE meh_items_shops SET Name='$ed_name', Items='$ed_items', Staff='$ed_staff', Upgrade='$ed_upgrade', House='0', Limited='$ed_limited', Founder='$ed_founder', Field='', Human='$human', Undead='$undead', Elf='$elf', Beast='$beast' WHERE id='$ed_id'");
              $insert_db->execute();
              if($insert_db){
          ...
          <form action="" method="POST">
            <table>
              ...
              <tr>
                ...
                <td class="pt-3 pl-3">
              <font>Item Race:</font>
              <select name="ed_race" class="form-control mb-3 bg-dark">
